...
var _ = Describe("Cache", Label("unit"), func() {
	type driverStub int
	var driverCache server.Cache[driverStub]
	var driverBuilders map[string]server.Builder[driverStub]

	BeforeEach(func() {
		driverCache = server.NewCache[driverStub]()
		driverBuilders = map[string]server.Builder[driverStub]{}
		for i := 0; i < 100; i++ {
			db := func(ctx context.Context, opts ...server.Option) (driverStub, error) {
